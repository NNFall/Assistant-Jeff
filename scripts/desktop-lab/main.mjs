...
const profile=fileURLToPath(new URL('../../work/desktop-lab/electron-profile/',import.meta.url));
...
if(!app.requestSingleInstanceLock()){app.quit();}else{
  ...
  app.whenReady().then(async()=>{
    ...
    for(const [name,handler] of Object.entries({start:()=>lab.start(),state:()=>lab.state(),run:payload=>lab.run(payload),stop:()=>lab.stop()})){
      ipcMain.handle(`lab:${name}`,async(event,payload)=>{
        ...
        try{return await handler(payload);}catch(error){return {error:/^[A-Z_]{1,60}$/.test(error.code)?error.code:'LAB_ERROR'};}
      });
    }
