...
$manifest = Get-Content -Raw -LiteralPath (Join-Path $source 'dependencies.json') | ConvertFrom-Json
$references = @()
foreach ($package in $manifest.packages) {
    ...
    if (!(Test-Path -LiteralPath $archive)) {
        Invoke-WebRequest -Uri "$($manifest.source)/$name/$($package.version)/$stem.nupkg" -OutFile $archive -UseBasicParsing
    }
    if ((Get-FileHash -LiteralPath $archive -Algorithm SHA256).Hash -ne $package.sha256) { throw "NuGet archive integrity mismatch: $stem" }
    # Re-extract each verified archive so stale or changed cached DLLs are not trusted.
...
& $compiler @common /target:winexe "/out:$(Join-Path $bin 'JeffDesktopLabTarget.exe')" (Join-Path $source 'Target.cs')
if ($LASTEXITCODE -ne 0) { throw 'Target compilation failed.' }
& $compiler @common @references /target:exe "/reference:$(Join-Path $framework 'System.Web.Extensions.dll')" "/reference:$(Join-Path $framework 'System.Management.dll')" "/reference:$(Join-Path $framework 'WPF\WindowsBase.dll')" "/out:$(Join-Path $bin 'JeffDesktopLabHelper.exe')" (Join-Path $source 'Helper.cs')
if ($LASTEXITCODE -ne 0) { throw 'Helper compilation failed.' }
foreach ($name in @('JeffDesktopLabTarget.exe','JeffDesktopLabHelper.exe')) { Copy-Item -LiteralPath (Join-Path $source 'app.config') -Destination (Join-Path $bin "$name.config") -Force }
Copy-Item -LiteralPath (Join-Path $source 'dependencies.json') -Destination $bin -Force
...
Write-Output "Built native UIA lab (not launched): $bin"
