...
const OPERATIONS = new Set(['inspect', 'activate', 'minimize', 'maximize', 'restore', 'close', 'select', 'invoke', 'toggle', 'expand', 'collapse', 'launch', 'set_language', 'set_text', 'new_tab']);
...
export function buildWindowsChoiceRequest(input) {
  ...
  criteria.done = 'An affirmative task exists; specific successful history establishes every required earlier step in order, and current observation establishes the latest applicable requested states. Earlier states intentionally superseded by later requested steps need not still hold. No remaining requested step. Inspection alone does not complete an action task.';
